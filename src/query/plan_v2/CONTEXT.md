...
- "alive Bind" vs "dead Bind" is a property derivable from the Alt's `introduces`: alive iff `sym ∈ chosen.introduces`. The code retains an `AliveTag` field on Alt and TopoEntry as a denormalisation of that property — the kind-dichotomy work did not delete it. Removing the tag would require routing `enode.children()[1]` into every dispatch point that currently reads `is_alive` (the builder's dead-Bind passthrough, the resolver's child-dispatch switch); that mechanical rework is deferred.
